...
contract OlympusStaking is OlympusAccessControlled {
     ...
     * @param _to address
     * @param _amount uint
     * @param _claimNow bool
     * @param _wantSohm bool
     * @return uint
     */
    function stake(
        address _to,
        ...
        if (_claimNow && warmupPeriod == 0) {
            return _send(_to, _amount, _wantSohm);
        } else {
    ...
    function unstake(
        ...
        if (_isSohm) {
            // means amount was returned in sohm form when staked.
            sOHM.safeTransferFrom(msg.sender, address(this), _amount);
            amount_ = amount_.add(bounty);
        } else {
            // means amount was returned in gohm form when staked.
        ...
        OHM.safeTransfer(_to, amount_); // finally, transfer ohm from this contract to the given address
    }
    ...
    function _send(
        ...
        if (_wantSohm) {
            // means return amount need to be in sOhms
            sOHM.safeTransfer(_to, _amount); // send as sOHM (equal unit as OHM)
            return _amount;
        } else {
            // means return amount needs to be in gOhms
